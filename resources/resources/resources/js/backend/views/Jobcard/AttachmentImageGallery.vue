<template>
  <div>
    <gallery
      :images="attachmentpictures"
      :index="indexgallery"
      @close="indexgallery = null"
    ></gallery>
    <template v-for="(image, imageInd) in modelattachmentpictures">
      <div
...
export default {
  ...
  methods: {
    async deleteWorkOrderImg (index) {
      ...
      if (confirm1) {
        var workorderImg = this.modelattachmentpictures
        var imgname = workorderImg[index].image_name
        // console.log(workorderImg[index].image_name)
        this.modelattachmentpictures.splice(index, 1)
        await axios
          ...
          .then((response) => {
            if (response.data.status === 200) {
              this.$emit('changeFile', this.modelattachmentpictures)
            }
          })
      }
    },
