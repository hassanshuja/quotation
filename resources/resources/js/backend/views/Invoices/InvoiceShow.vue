            ...
            <b-col sm="6">
              <b-col sm="8">
                <div class="well" style="border:1px solid #CCCCCC">
                  <address class="form-group">
                    <label class="control-label"><b>Invoice for:</b></label>
...
export default {
    ...
    'last_Invoice_ref': function (val) {
      if (val) {
        ...
        if (this.settings.Invoice_ref_start) {
          this.model.quotation_number = this.settings.Invoice_ref_start + 1
        } else {
  ...
  methods: {
    ...
    downloadjspdf: function () {
      ...
      var source = window.document.getElementById('Invoices-view')
      doc.fromHTML(
        source,
        15,
        15
    ...
    async getInvoicesReference () {
      ...
      this.last_Invoice_ref = data.quotation_number
    },
    showModal (ModalRef) {
    ...
    print: function (div) {
      ...
      // newWin.print()
      // newWin.close()
      var printWindow = window.open('/admin/Invoices/3/printpdf ', 'popupWindow', 'width=600,height=600,scrollbars=yes')
      printWindow.print()
    }
  }
}
